...
t = 0.
ambient = np.array([0., 0., 0.], dtype=GLfloat)
diffuse = np.array([0., 0., 0.], dtype=GLfloat)
...
def init_shaders():
    ...
    glUniform3f(locations[b"diffuse"], 1.0, 0.0, 0.0)
...
def draw():
    ...
    ambient[:] = abs(np.sin([t, t + 1, t + 2]))
    diffuse[:] = abs(np.sin([t, t + 1, t + 2]))

    glUniform3f(locations[b"ambient"], ambient[0], ambient[1], ambient[2])
    glUniform3f(locations[b"diffuse"], diffuse[0], diffuse[1], diffuse[2])
    glutPostRedisplay()
...
def main():
    ...
    glutCreateWindow("Laboratory Work #6")
